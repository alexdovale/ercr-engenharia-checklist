...
const UIRender = {
  ...
  // Rodapé exato do documento oficial
  prFooterHTML: () => {
  ...
  // Renderiza apenas o miolo da assinatura para colocar em cima da linha
  ...
  buildPrintReport: (sectionsArray, signatureData, currentSeq, logoB64, cnpj) => {
    ...
    html += `</div>`; // Fim da Página 1 (As quebras ocorrem naturalmente pelo CSS)

    // Página de Não Conformidades (14)
    ...
    html += UIRender.prFooterHTML();
    html += `</div>`; // Fim da Página 2

    container.innerHTML = html;
  },

  buildReceiptReport: (currentSeq, logoB64, cnpj) => {
    // ... mantive o recibo exatamente como você tinha feito
    const v = id => document.getElementById(id)?.value.trim() || '';
    const esc = s => (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
